...
def get_opt():
    ...
    parser.add_argument('--cloth_name', type=str, required=True, help="Name of the cloth image")
    parser.add_argument('--img_name', type=str, required=True, help="Name of the person image")
...
def test(opt, seg, gmm, alias):
    ...
    test_dataset = VITONDataset(opt, opt.cloth_name, opt.img_name)
...
def main():
    ...
    # Pass cloth_name and img_name to the dataset
    test_dataset = VITONDataset(opt, opt.cloth_name, opt.img_name)
    test_loader = VITONDataLoader(opt, test_dataset)
